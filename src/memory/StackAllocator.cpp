...
#include <cstdlib>
#ifdef STACK_ALLOCATOR_SET_MEMORY
#include <cstring>
#endif
#include <iostream>// TODOJM: Remove

std::atomic_int StackAllocator::m_NrOfCreatedStacks( 0 );

StackAllocator& StackAllocator::GetThreadStack() {
	static thread_local StackAllocator stackAllocator( STACK_ALLOCATOR_SIZE );
...
#ifdef STACK_ALLOCATOR_COUT_CONSTRUCTION
...
#ifdef STACK_ALLOCATOR_SET_MEMORY
	memset( m_Memory, STACK_ALLOCATOR_MEMORY_INITIALIZE_VALUE, stackSize );
#endif
}
...
#ifdef STACK_ALLOCATOR_COUT_DESTRUCTION
	std::cout << "destructed stack" << std::endl;
#endif
}

void* StackAllocator::Allocate( size_t allocationSize, size_t alignment ) {
...
#ifdef STACK_ALLOCATOR_SET_MEMORY
	memset( m_Memory + m_Marker, STACK_ALLOCATOR_MEMORY_ALLOCATION_VALUE, alignedAllocationSize );
#endif
...
void StackAllocator::Unwind( Marker marker ) {
#ifdef STACK_ALLOCATOR_SET_MEMORY
	memset( m_Memory + marker, STACK_ALLOCATOR_MEMORY_FREE_VALUE, m_Marker - marker );
#endif
	m_Marker = marker;
}

uint8_t* StackAllocator::GetBuffer() {
	return m_Memory;
}
...
int StackAllocator::GetNrOfStacks() {
