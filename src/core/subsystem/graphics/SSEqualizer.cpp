...
void SSEqualizer::Startup( SubsystemCollection* const subsystemCollection ) {
	...
	FMOD_System_CreateStream(m_SoundSystem, "../../../asset/music/Vee.mp3", FMOD_LOOP_NORMAL | FMOD_2D | FMOD_HARDWARE | FMOD_UNIQUE, nullptr, &m_Song);
...
void SSEqualizer::UpdateUserLayer( const float deltaTime ) {
	...
	int k = 0;
	for (int i = 0; i < SPECTRUM_SIZE / 4; ++i) {
		float h = glm::max((rightSpectrum[i] + rightSpectrum[i]) * 5.0f - 0.04f, 0.0f) + 0.015f;
		particles[k].Pos = glm::vec4((k / (float)SPECTRUM_SIZE) * 2.0f - 1.0f, h, 0.1f, 1);
		particles[k].Color = glm::vec4(1);
		k++;
		particles[k].Pos = glm::vec4((k / (float)SPECTRUM_SIZE) * 2.0f - 1.0f, h, 0.1f, 1);
		particles[k].Color = glm::vec4(1);
		k++;
	...
	glBindVertexArray(m_VAO);
	glPointSize(4.0f);
	glDrawArrays(GL_POINTS, 0, SPECTRUM_SIZE);
