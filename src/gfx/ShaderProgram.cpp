...
void gfx::ShaderProgram::CreateShader ( std::ifstream* FileStream, GLenum shaderType, const std::string& filename, bool print ) {
	...
	while ( !end ) {
		...
		if ( found != std::string::npos ) {
			...
			std::string str = GetDirectoryFromFilePath ( std::string ( filename ) ) + "/" + s;
...
void gfx::ShaderProgram::CreateShader ( std::ifstream* FileStream, GLenum shaderType, const std::string& filename, bool print, ShaderDefineContainer& container ) {
	std::string line;
	size_t found;
	std::string shaderText;
	getline(*FileStream, line);
	...
	while ( !end ) {
		...
		if ( found != std::string::npos ) {
			...
			std::string str = GetDirectoryFromFilePath(std::string(filename.c_str())) + s;
			shaderText += LoadText ( str.c_str() );
			shaderText += "\n";
		} else {
			found = line.find ( "#end_shader" );
