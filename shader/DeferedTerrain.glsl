...
layout(binding=1) uniform 	sampler2D 	g_RoughnessTex;

uniform		vec3		g_Color;
uniform 	float		g_Tiling;
...
void main(){
	...
	vec2 texCoord = TexOut.xy * g_Tiling;
	float roughness = texture(g_RoughnessTex,texCoord.xy).r;
	float metallic =  0.0f;
	const float minRoughness=1e-4;
	roughness = max(minRoughness, roughness);

	vec3 normal = normalize(NormalW.xyz); //CalcBumpedNormal(NormalW.xyz, TangentW.xyz, g_NormalTex, texCoord.xy);
	vec4 albedo = texture(g_DiffuseTex, texCoord.xy);
	if(albedo.a <= 0.01f){
		discard;
	}
